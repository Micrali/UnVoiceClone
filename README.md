...
系统围绕“公开音视频发布前的声纹主动保护”设计，包含网页端交互界面、Flask API 服务、音视频处理流程、声纹特征扰动模块、身份替换策略、鲁棒优化模块、配置文件与测试用例。前端负责完成音频/视频上传、防御结果展示和克隆测试交互；后端负责文件处理、任务调度与算法模块调用；核心算法部分实现动态说话人编码、KL 散度身份替换、WaveGlow 对抗生成、基音调节和扰动优化等关键步骤。
...
| 登录页 | `http://127.0.0.1:8080/auth.html` | 系统登录入口 |
| 控制台 | `http://127.0.0.1:8080/dashboard.html` | 处理指标与任务记录看板 |
| 功能展示 | `http://127.0.0.1:8080/function.html` | 系统核心功能入口 |
| 音频防御 | `http://127.0.0.1:8080/audio-defend.html` | 音频主动防御流程 |
| 视频防御 | `http://127.0.0.1:8080/video-defend.html` | 视频人声防护流程 |
| 音频转换测试 | `http://127.0.0.1:8080/audio-transform.html` | VC 克隆攻击测试 |
| 音频合成测试 | `http://127.0.0.1:8080/audio-synthesis.html` | TTS 克隆攻击测试 |
| 兼容测试入口 | `http://127.0.0.1:8080/test-defend.html` | 跳转到音频转换测试 |
...
- 支持 MP3、WAV 音频上传
- 输入音频样例：`original_voice_demo.wav`
- 输出防御音频：`unvc_defended_voice.wav`
- 展示原始音频与防御后音频波形
- 输出防御成功率、语音保真相似度、抗压缩防御能力
- 点击“开始防御”即可查看处理结果、指标和风险评估

### 视频人声防护

- 支持 MP4、AVI 视频文件处理
- 执行视频音轨提取与人声片段定位
- 输入视频样例：`interview_clip_demo.mp4`
- 输出防御视频：`unvc_defended_video.mp4`
- 展示原始视频与防御后视频预览
- 页面展示人声片段数量、处理时长与克隆风险变化

### 音频转换测试

- 模拟攻击者基于目标音色进行语音转换克隆
- 源音频样例：`speaker_source_demo.wav`
- 目标音色样例：`defended_target_demo.wav`
- 转换输出样例：`clone_result_demo.wav`
